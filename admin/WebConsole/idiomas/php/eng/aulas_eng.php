<?php
//________________________________________________________________________________________________________
//
//	Php language file: aulas_eng.php
//	Language: English
//________________________________________________________________________________________________________
	$TbMsg=array();
	$TbMsg[0]='"New labs group"';
	$TbMsg[1]='"Computer status"';
	$TbMsg[2]='"Switch"';
	$TbMsg[3]='"Add new lab"';
	$TbMsg[4]='"Refresh"';
	$TbMsg[5]='"Commands"';
	$TbMsg[6]='"Action queue"';
	$TbMsg[7]='"Delete labs group"';
	$TbMsg[8]='"New computers group"';
	$TbMsg[9]='"Add new computer"';
	$TbMsg[10]='"Configuration"';
	$TbMsg[11]='"Set computer"';
	$TbMsg[12]='"Computer template"';
	$TbMsg[13]='"Properties"';
	$TbMsg[14]='"Delete lab"';
	$TbMsg[15]='"Rename"';
	$TbMsg[16]='"Delete computers group"';
	$TbMsg[17]='"Move computer"';
	$TbMsg[18]='"Delete computer"';
	$TbMsg[19]='"Tasks"';
	$TbMsg[20]='"Jobs"';
	$TbMsg[21]='"All actions"';
	$TbMsg[22]='Computer Status';
	$TbMsg[23]='Computer Lab';
	$TbMsg[24]='Scope: Centers-OU';
	$TbMsg[25]='Scope: Labs group';
	$TbMsg[26]='Scope: Computers Group ';
	$TbMsg[27]='"Add computers"';
	$TbMsg[28]='"Procedures"';
	$TbMsg[29]='"Book a computer Lab"';
	$TbMsg[30]='"Confirmed"';
	$TbMsg[31]='"Pending"';
	$TbMsg[32]='"Denied"';
	$TbMsg[33]='"Remote console"';
	$TbMsg[34]='"Reserve schedules"';
	$TbMsg[35]='"Operators"';
	$TbMsg[36]='"Deleteoperator"';
	$TbMsg[37]='"Add new operator"';
	$TbMsg[38]='"Wizards"';
	$TbMsg[39]='"Console echo"';
	$TbMsg[40]='"Advanced NetBoot"';
	$TbMsg[41]='"Relocate computers"';
	$TbMsg[42]='Advanced Boot Management';
	$TbMsg[43]='"Save"';
	$TbMsg[44]='Computer allocator';
	$TbMsg[45]='Scope name';
	$TbMsg[46]='Scope id';
	$TbMsg[47]='"Historical log"';
	$TbMsg[48]='"Real time log"';
	$TbMsg[49]='"Synchronization';
	$TbMsg[50]='"Synchronization 2';
