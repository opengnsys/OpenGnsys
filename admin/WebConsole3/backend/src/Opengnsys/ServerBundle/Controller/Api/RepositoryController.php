...
/**
 * @RouteResource("Repository")
 */
class RepositoryController extends ApiController
{	
	
	/**
	 * Options a Repository from the submitted data.
	 *
	 * @ApiDoc(
	 *   resource = true,
	...
	public function optionsAction(Request $request){
		$array = array();
		$array['class'] = RepositoryType::class;
		$array['options'] = array();
	 ...
	 *
	 * @Annotations\View(templateVar="Repository", serializerGroups={"opengnsys_server__repository_cget"})
	 *
	 * @param Request               $request      the request object
	...
	public function cgetAction(Request $request, ParamFetcherInterface $paramFetcher)
	{
		...
		$objects = $this->container->get('opengnsys_server.api_repository_manager')->all($limit, $offset, $matching);
	 ...
	 * )
	 *
	 * @Annotations\View(templateVar="repository", serializerGroups={"opengnsys_server__repository_get"})
	 *
	 * @param int     $slug      the repository id
	 *
	 * @return array
	 *
	 * @throws NotFoundHttpException when repository not exist
	 */
	...
	/**
	 * Create a Repository from the submitted data.
	 *
	 * @ApiDoc(
	 *   resource = true,
	 ...
	 * @Annotations\View(
	 *  template = "object",
	 *  serializerGroups={"opengnsys_server__repository_get"},
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_repository_manager')->post(
					$request->request->all()
			);	
	...
	/**
	 * Update existing Repository from the submitted data or create a new Repository at a specific location.
	 *
	 * @ApiDoc(
	 *   resource = true,
	 *   input = {"class" = "opengnsys_server__api_form_type_repository", "name" = ""},
	...
	{
		try {
			if (!($object = $this->container->get('opengnsys_server.api_repository_manager')->get($slug))) {
				$statusCode = Response::HTTP_CREATED;
				$object = $this->container->get('opengnsys_server.api_repository_manager')->post(
						$request->request->all()
				);
				...
				$object = $this->container->get('opengnsys_server.api_repository_manager')->put(
						$object,
						$request->request->all()
				);
			}
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_repository_manager')->patch(
					$this->getOr404($slug),
					$request->request->all()
			);
	...
	public function deleteAction($slug)
	{
		...
		$object = $this->container->get('opengnsys_server.api_repository_manager')->delete($object);	
	
		return $this->view(null, Response::HTTP_NO_CONTENT);
	}
	
	/**
	 * Fetch a Repository or throw an 404 Exception.
	 *
	 * @param mixed $slug
	 *
	 * @return Repository
