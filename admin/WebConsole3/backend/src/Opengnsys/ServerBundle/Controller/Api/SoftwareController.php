...
{	
	...
	public function cgetAction(Request $request, ParamFetcherInterface $paramFetcher)
	{
		...
		$objects = $this->container->get('opengnsys_server.api_software_manager')->all($limit, $offset, $matching);
	 ...
	 *   input = {"class" = "opengnsys_server__api_form_type_software", "name" = ""},
	 *   statusCodes = {
	 *     200 = "Returned when successful",
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_software_manager')->post(
					$request->request->all()
			);	
	...
	{
		try {
			if (!($object = $this->container->get('opengnsys_server.api_software_manager')->get($slug))) {
				$statusCode = Response::HTTP_CREATED;
				$object = $this->container->get('opengnsys_server.api_software_manager')->post(
						$request->request->all()
				);
				...
				$object = $this->container->get('opengnsys_server.api_software_manager')->put(
						$object,
						$request->request->all()
				);
			}
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_software_manager')->patch(
					$this->getOr404($slug),
					$request->request->all()
			);
	...
	public function deleteAction($slug)
	{
		...
		$object = $this->container->get('opengnsys_server.api_software_manager')->delete($object);	
