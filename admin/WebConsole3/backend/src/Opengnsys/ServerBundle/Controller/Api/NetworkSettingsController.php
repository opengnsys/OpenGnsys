...
{	
	...
	public function cgetAction(Request $request, ParamFetcherInterface $paramFetcher)
	{
		...
		$objects = $this->container->get('opengnsys_server.api_network_settings_manager')->all($limit, $offset, $matching);
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_network_settings_manager')->post(
					$request->request->all()
			);	
	...
	{
		try {
			if (!($object = $this->container->get('opengnsys_server.api_network_settings_manager')->get($slug))) {
				$statusCode = Response::HTTP_CREATED;
				$object = $this->container->get('opengnsys_server.api_network_settings_manager')->post(
						$request->request->all()
				);
				...
				$object = $this->container->get('opengnsys_server.api_network_settings_manager')->put(
						$object,
						$request->request->all()
				);
			}
	...
	{
		try {
			$object = $this->container->get('opengnsys_server.api_network_settings_manager')->patch(
					$this->getOr404($slug),
					$request->request->all()
			);
	...
	public function deleteAction($slug)
	{
		...
		$object = $this->container->get('opengnsys_server.api_network_settings_manager')->delete($object);	
