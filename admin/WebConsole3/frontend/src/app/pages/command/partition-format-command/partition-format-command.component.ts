...
export class PartitionFormatCommandComponent implements OnInit {
    ...
    convertPartitionType(partition) {
        if (this.partTableTypeIsMSDOS()) {
            if (this.ogCommonService.isWINDOWS(partition)) {
                partition.type = 'NTFS';
            } else if (this.ogCommonService.isUNKNOWN(partition)) {
                partition.type = 'NTFS';
            }
        } else if (this.partTableTypeIsGPT()) {
            if (this.ogCommonService.isWINDOWS(partition)) {
                partition.type = 'WINDOWS';
            } else if (this.ogCommonService.isDATA(partition)) {
        ...
        if (this.partTableTypeIsMSDOS()) {
            if (this.diskConfig.partitions.length > 5) {
                ...
                this.diskConfig.partitions.forEach(function (partition, index) {
                    ...
                    if (index < numParts || self.ogCommonService.isFreeSpace(partition) || self.ogCommonService.isCACHE(partition)) {
            ...
            if (this.partTableTypeIsGPT()) {
                ...
                if (!this.ogCommonService.isEFI(this.diskConfig.partitions[0])) {
                    tmpPartitions.push({
                        type: 'EFI',
                        size: 512000,
    ...
    reorderPartitions() {
        ...
        let indexCache = -1;
        this.diskConfig.partitions.forEach(function (partition, index) {
            if (partition.type !== 'free_space' && !self.ogCommonService.isCACHE(partition)) {
                partition.partition = (tmpPartitions.length + 1);
                tmpPartitions.push(partition);
            } else if (self.ogCommonService.isCACHE(partition)) {
                indexCache = index;
    ...
    */
    ...
    checkPartitionType(partition, parentPartition) {
        let ok = true;
        if (this.ogCommonService.isCACHE(partition)) {
        ...
        } else if (this.ogCommonService.isEXTENDED(partition)) {
        ...
        if (ok) {
            if (parentPartition) {
                this.updateExtendedPartitionsUsage(parentPartition);
            } else {
...
// document.fdatosejecucion.atributos.value="scp="+escape(document.fdatos.codigo.value)+RC;
    ...
    generateOgInstruction() {
        const self = this;
        this.ogCommandsService.execution.sendConfig = true;
        ...
        this.diskConfig.partitions.forEach(function (partition, index) {
            if (partition.type !== 'free_space') {
                ...
                if (!self.ogCommonService.isCACHE(partition)) {
