(function(){
	...
	OusController.$inject = ['$rootScope', '$scope','$filter', '$state', 'GroupsResource', 'OusResource', 'ClientsResource'];

	function OusController($rootScope, $scope, $filter, $state, GroupsResource, OusResource, ClientsResource) {
		var vm = this;
	  		...
	  		if($state.$current.name === "app.labs"){
		  		GroupsResource.query({ouid: $rootScope.user.ou.id}).then(
		  			function(result){
		  				...
		  				OusResource.query({ouid: $rootScope.user.ou.id}).then(
		  					function(result){
		  						...
		  						_.each(result, function(lab){
		  							...
		  							OusResource.clients({ouid: $rootScope.user.ou.id, labid: lab.id}).then(
		  								function(clients){
