(function(){
	...
	EditMenuController.$inject = ['$rootScope', '$scope','$state', '$timeout', '$filter', '$sce', 'toaster','MenusResource'];

	function EditMenuController($rootScope, $scope, $state, $timeout, $filter, $sce, toaster, MenusResource) {
		var vm = this;
		vm.menu = [];
		vm.formOptions = {};
		vm.save = save;
		...
		function init(){
			if($rootScope.user){
				MenusResource.get({menuId: $state.params.menuId}).then(
					function(response){
						vm.menu = response;
						vm.menu.resolution = {id: vm.menu.resolution};
					},
					function(error){
		...
		function save(Form){
            ...
            // Si no hubo ningun error se guardan todas las pgms
           	if(result == true){
	  			...
	  			MenusResource.update({menuId: vm.menu.id},menuCopy).then(
		  			function(response){
