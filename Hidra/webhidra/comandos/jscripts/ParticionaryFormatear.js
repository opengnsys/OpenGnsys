...
// Nombre del fichero: ParticionaryFormatear.js
// Descripci� : 
//		Este fichero implementa las funciones javascript del fichero ParticionaryFormatear.php (Comandos)
// *************************************************************************************************************************************************
...
//________________________________________________________________________________________________________
 function confirmar(){
	if (comprobar_datos()){
		...
		if(swenv){
			var wurl="./gestores/gestor_ParticionaryFormatear.php"
...
function annadir_particion(idc){
	...
	textHtml=oTDs[0].innerHTML     //  Toma el   código html a sustituir

	oTABLE=document.getElementById("patron_contenidoparticion") 
	var wpatrontablaparticion=oTABLE.innerHTML     //  Toma la código html a añadir

	// Calcula el número de la última partición añadida
	oINPUT=document.getElementById("ultpa_"+idc) 
	var wultpa=parseInt(oINPUT.value);
	wultpa++;
	oINPUT.value=wultpa; // Lo almacena
	ultpa=oINPUT.value; 

	//Sustituye parametros en la cadena html a añadir
	...
	for (var posa=posb;posa>=0;posa--) {
		etq=textHtml.substr(posa,5);
 		if ("</TR>" == etq.toUpperCase())	break; // Retrocede buscando etiqueta </TR>
	 }
...
//	Comprobar retorno
//________________________________________________________________________________________________________
function resultado_ParticionaryFormatear(resul){
	if (!resul){
		alert(CTbMsg[1]);	
		return
	}
