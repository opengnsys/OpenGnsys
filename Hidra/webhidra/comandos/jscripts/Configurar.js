	...
	if (comprobar_datos()){
		...
		for(var x=0;x<nuevasipes.length;x++){
				...
				for(var i=1;i<oTRs.length;i++){ // recorre TR's de las particiones
					...
					var inputtama=oTDs[4].childNodes[1] // recupera el tamaño
...
function chgpar(o){
	...
	swenvio.value=1; // marca la partición para ser tratada en el envío de trama  
}
//________________________________________________________________________________________________________
...
function chgtama(idc){
		...
		for(var i=1;i<oTRs.length;i++){ // recorre TR's de las particiones
			...
			if(desplepar.selectedIndex!=0 && desplepar.selectedIndex!=5){ // Si la particion no esta vacia
				...
				oTABLE.value=1; // marca la partición para ser tratada en el envío de trama  
			}
		}
}
//________________________________________________________________________________________________________
...
function chgtotal(op){
	...
	for(var i=1;i<oTRs.length;i++){ // recorre TR's de las particiones
		...
		var littiposo=oTDs[3].childNodes[0]
		if(despletipopar.selectedIndex==0 || despletipopar.selectedIndex==5) // partición está vacíao es swap no puede llevarse a cabo ningún tipo de acción sobre ella
			continue
		if (littiposo.value==1) // Está partición debe ser necesariamente formateada porque se ha cambiado el S.O. 
			continue
		...
		despleacc.selectedIndex=op; // Coloca la acción en el desplegable
		oTABLE.value=1; // marca la partición para ser tratada en el envío de trama  
	}
}